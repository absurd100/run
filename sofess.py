import logging, html, os, json, sys, subprocess, asyncio, signal, requests
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if uid_int == OWNER_ID:
        ...
        if text == '📋 TUTORIAL':
            try:
                ...
                await update.message.reply_text(f"Koneksi Error: {e}"); return
    ...
    # PROSES KIRIM MENFESS (TEKS / FOTO / VIDEO)
    ...
    if not text.startswith('/') and text not in list_tombol:
        ...
        mode = context.user_data.get('mode', 'anonim')
        ...
        isi_menfess = text if (not msg.photo and not msg.video) else (msg.caption if msg.caption else "(Media)")
        ...
        try:
            # LOGIKA PENDETEKSI VIDEO/FOTO/TEKS
            if msg.photo:
                snt = await context.bot.send_photo(target, msg.photo[-1].file_id, caption=final_text, parse_mode=ParseMode.HTML)
            elif msg.video:
                snt = await context.bot.send_video(target, msg.video.file_id, caption=final_text, parse_mode=ParseMode.HTML)
            else:
                snt = await context.bot.send_message(target, text=final_text, parse_mode=ParseMode.HTML, link_preview_options=LinkPreviewOptions(is_disabled=True))
            ...
            elif msg.video: await context.bot.send_video(OWNER_ID, msg.video.file_id, caption=f"📩 LOG: {uid}", reply_markup=log_kb, parse_mode=ParseMode.HTML)
